...
def dashboard():
    ...
    if "username" in session and "streamkey" in session:
        ...
        return render_template("dashboard.html", username=username, streamkey=streamkey, url=URL)
...
def user_stream(streamer_username):
    ...
    if "username" in session and "streamkey" in session:
        ...
        if found_user:
            # if found, route user streamer's live stream
            return render_template("user-stream.html", username=streamer_username, key=found_user.streamkey, url=URL)
        else: # otherwise, send to dashboard with an error
